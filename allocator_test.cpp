...
public:
    ...
    size_type max_size() const {
        return (30000000 - offset) / sizeof(T);
    }

    pointer allocate(size_type num, const void * = nullptr) {
        last = true;
        auto ret = static_cast<pointer>(static_cast<void *>(static_cast<char *>(stack) + offset));
        offset += sizeof(T) * num;
        return ret;
    }
...
private:
    ...
    array<array<array<int, n + 1>, n + 1>, n> c{};
    ...
    void dfs_comp_greater(array<array<int, n + 1>, n + 1> &comp, int cur, int priv) {
        for (int i = 0; i != n + 1; ++i) {
            if (comp[priv][i] == 1) {
                comp[cur][i] = 1;
                comp[i][cur] = -1;
    ...
    void apply_cond_cur(array<array<int, n + 1>, n + 1> &comp, int a, int b) {
        comp[a][b] = -1;
        comp[b][a] = 1;
        dfs_comp_greater(comp, b, a);
    }

    void make_cond(array<array<int, n + 1>, n + 1> &comp, array<array<array<int, n + 1>, n + 1>, n> &count,
                   vector<bool, MyAlloc<bool>> &out_check, int player, int a, int b) {
        array<array<int, n + 1>, n + 1> copy(comp);
    ...
    bool is_ne_rec(array<array<array<int, n + 1>, n + 1>, n> comp, array<array<array<int, n + 1>, n + 1>, n> count,
                   int player, int a, int b, vector<bool, MyAlloc<bool>> out_check) {
        ++num;
    ...
    bool is_ne() {
        array<array<array<int, n + 1>, n + 1>, n> comp{}, count{};
        for (int i = 0; i != dif; ++i) {
            for (int j = 0; j != n + 1; ++j) {
                for (int k = 0; k != n + 1; ++k) {
        ...
        vector<bool, MyAlloc<bool>> out_check(form.get_size());
        do {
            if ((count[player][a][b] != 0 || count[player][b][a] != 0) && comp[player][a][b] == 0) {
                if (!is_ne_rec(comp, count, player, a, b, out_check)) {
                    return false;
