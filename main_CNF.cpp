...
private:
    ...
    void write_CNF(int p_num, vector<int> &players, int beg) {
        int var = dif * (n + 1) * n;
        fstream f("/home/nikolay/Desktop/data/" + to_string(n) + "/" + to_string(dif) + "/" + to_string(num) + "_" +
                  to_string(p_num) + ".txt", ios::out);
...
private:
    ...
    void find_gf(vector<v> &graph) {
        ...
        if (check_connected(graph) && !check_iso(graph)) {
            ...
            fstream f("/home/nikolay/Desktop/graphs/" + to_string(num) + ".txt", ios::out);
            for (auto &line : graph) {
                for (int v : line.out) {
                    f << v << ' ';
                }
    ...
    void make_graph(vector<v> vertex, int pos) {
        ...
        if (pos == n) {
            ...
            for (int i = 0; i != uniq.size(); ++i)
                gen_subsets(p, uniq, vertex[pos].degree, i, vector<int>());
            for (auto &perm : p) {
...
public:
    g() : num(0), all(0), d_count(0), pool(thread::hardware_concurrency()) {
            ...
            for (int j = 0; j != i; ++j) {
                for (int l = 1; l != n + 1; ++l) {
                    for (int m = l + 1; m != n + 1; ++m) {
                        vector<vector<int>> CNF;
                        fstream f("/home/nikolay/Desktop/CNF/" + to_string(n) + "/" + to_string(i) + "/" + "lims/" +
                                  to_string(j) + "_" + to_string(l) + "_" + to_string(m) + ".txt", ios::out);
                        for (int k = 0; k != i; ++k) {
                            if (k != j) {
                                for (int i1 = 1; i1 != n + 1; ++i1) {
                                    CNF.push_back({toLit(k, 0, i1, n)});
                                }
                            } else {
                                CNF.push_back({toLit(k, l, 0, n)});
                                CNF.push_back({toLit(k, m, 0, n)});
                                for (int i1 = 1; i1 != n + 1; ++i1) {
                                    if (i1 != l && i1 != m) {
                                        CNF.push_back({toLit(k, 0, i1, n)});
                                    }
        ...
        for (int dif = 1; dif != n + 1; ++dif) {
            vector<vector<int>> CNF;
            fstream f("/home/nikolay/Desktop/CNF/" + to_string(n) + "/" + to_string(dif) + "/" + "ter/op.txt",
                      ios::out);
            for (int i = 0; i != dif; ++i) {
                for (int j = 0; j != n + 1; ++j) {
