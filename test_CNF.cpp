...
private:
    ...
    void addTr(int player, int a, int b, int c, vector<vector<int>> &CNF) {
        CNF.push_back({toLit(player, b, a), toLit(player, c, b), toLit(player, a, c)});
        CNF.push_back({toLit(player, a, b), toLit(player, b, c), toLit(player, c, a)});
    }

    void write_CNF(int p_num) {
        int var = dif * (n + 1) * n;
        fstream f("data/" + to_string(graph.size()) + "/" + to_string(num) + "_" + to_string(p_num) + ".txt", ios::out);
        vector<vector<int>> CNF;
        for (int i = 0; i != dif; ++i)
            for (int j = 1; j != n + 1; ++j)
                CNF.push_back({toLit(i, 0, j)});
        for (int i = 0; i != dif; ++i)
            for (int j = 0; j != n + 1; ++j)
        ...
        for (auto &c : out_comp) {
            CNF.emplace_back();
            for (auto &p : c) {
                CNF.back().push_back(toLit(p.player, p.a, p.b));
            }
        }
        var /= 2;
