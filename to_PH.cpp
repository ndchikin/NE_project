...
public:
    ...
    void find_cases(vector<uint64_t> &index, vector<uint8_t> &outcomes, vector<vector<op>> &out_comp) {
...
public:
    gf(vector<v> g, vector<int> pl, int beg) : beg(beg), n(g.size()), dif(0), graph(move(g)),
                                                    players(move(pl)) {
        vector<int> count(n);
        for (int i = 0; i != n; ++i) {
            if (!count[players[i]]) {
    ...
    void make_cnf() {
        vector<int> strategy(n), cl(n);
        vector<uint8_t> outcomes(n + 1);
        vector<uint64_t> index(dif);
        graph[beg].has_t = false;
        size.assign(dif, 1);
        for (int i = 0; i != n; ++i) {
        ...
        ndim form(size);
        do {
            form[find_index(strategy)] = find_outcome(strategy, cl, beg);
            cl.assign(n, 0);
        } while (incr_g(strategy));
        out_comp.resize(form.get_size());
        do {
            form.find_cases(index, outcomes, out_comp);
        } while (incr_s(index));
    }
    ...
    void write_cnf() {
        ...
        for (uint64_t i = 0; i != out_comp.size(); ++i) {
            if (!out_comp[i].empty()) {
                result.emplace_back();
                for (uint64_t j = 0; j != out_comp[i].size(); ++j) {
                    result.back().push_back(toLit(out_comp[i][j].player, out_comp[i][j].a, out_comp[i][j].b));
                }
